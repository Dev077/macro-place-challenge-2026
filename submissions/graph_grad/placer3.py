...
def soft_density_spread(
    ...
    """
    TILOS-style top-10% density penalty computed over soft macros only.

    Hard macros are fixed, so pushing them is pointless.  This term creates a
    gradient that repels soft macros from each other and from dense clusters,
    spreading them into the free space between hard macros.

    Returns the same scale as ``tilos_density_loss`` (0.5 × top-10% mean
    density) but restricted to the soft-macro slice of the population.
    """
    K, N, _ = pop.shape
    n_soft = N - n_hard
    if n_soft == 0:
        return torch.zeros(K, device=pop.device, dtype=pop.dtype)

    soft_pop = pop[:, n_hard:, :]   # [K, n_soft, 2]
    soft_sizes = sizes[n_hard:]     # [n_soft, 2]
    ...
    hw = soft_sizes[:, 0] / 2
    hh = soft_sizes[:, 1] / 2
    macro_l = (x - hw.unsqueeze(0)).unsqueeze(-1)   # [K, n_soft, 1]
    macro_r = (x + hw.unsqueeze(0)).unsqueeze(-1)
    macro_b_ = (y - hh.unsqueeze(0)).unsqueeze(-1)
    macro_t_ = (y + hh.unsqueeze(0)).unsqueeze(-1)
    col_lb = col_l.view(1, 1, grid_col)
...
class GraphGradPlacer:
    ...
    def __init__(
        ...
        soft_spread_weight: float = 0.5,   # Weight for soft-macro-only density spread term
    ):
        self.seed = seed
        ...
        self.soft_spread_weight = soft_spread_weight
    ...
    def _soft_only_single_run(self, benchmark: Benchmark, run_seed: int):
        ...
        for step in range(n_steps):
            ...
            # Soft-macro spread: ramp from 0→full over the first 20% of steps,
            # then hold constant.  Lets WL settle before imposing spread pressure.
            spread_w = self.soft_spread_weight * min(1.0, progress / 0.2)
            spread = soft_density_spread(pop, sizes_all, n_hard, grid_col, grid_row, cw, ch)
            proxy_surr = wl_n + 0.5 * dens + 0.5 * cong  # mirrors compute_proxy_cost
            loss = (proxy_surr + spread_w * spread).sum()
            loss.backward()
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            spread = soft_density_spread(pop, sizes_all, n_hard, grid_col, grid_row, cw, ch)
            surr = wl_n + 0.5 * dens + 0.5 * cong + self.soft_spread_weight * spread
