...
def _build_soft_to_hards_index(benchmark: Benchmark) -> List[List[int]]:
    """
    For each soft macro, gather the indices of all hard macros it's
    net-connected to (de-duped).  Used by ``_init_soft_at_centroids`` to
    place soft cells at the centroid of their connected hards instead of
    canvas-center jitter.
    """
    n_hard = benchmark.num_hard_macros
    n_macros = benchmark.num_macros
    n_soft = n_macros - n_hard
    soft_to_hards: List[set] = [set() for _ in range(n_soft)]
    for net_pins in benchmark.net_pin_nodes:
        owners = net_pins[:, 0].tolist()
        hards_in_net = [int(o) for o in owners if int(o) < n_hard]
        softs_in_net = [int(o) for o in owners if n_hard <= int(o) < n_macros]
        if not hards_in_net:
            continue
        for s in softs_in_net:
            soft_to_hards[s - n_hard].update(hards_in_net)
    return [sorted(list(s)) for s in soft_to_hards]


def _init_soft_at_centroids(
    hard_positions: np.ndarray,
    soft_to_hards: List[List[int]],
    cw: float,
    ch: float,
) -> np.ndarray:
    """
    Initialize each soft macro at the centroid of its connected hard macros.

    Falls back to canvas centre for any soft macro with no hard connections.
    Returns ``[n_soft, 2]`` positions.
    """
    n_soft = len(soft_to_hards)
    soft_pos = np.full((n_soft, 2), (cw / 2.0, ch / 2.0), dtype=np.float64)
    for s, hards in enumerate(soft_to_hards):
        if hards:
            soft_pos[s] = hard_positions[hards].mean(axis=0)
    return soft_pos


def _optimize_orientations(
    benchmark: Benchmark,
    placement_full_np: np.ndarray,
    max_passes: int = 3,
) -> Tuple[np.ndarray, List[np.ndarray]]:
    """
    Klein-4 orientation optimization per hard macro.

    For each hard macro, try the four legal orientations (N, FN, FS, S) and
    pick the one that minimizes the macro's contribution to total HPWL.
    Iterate until no orientation changes.

    Macro center, width and height are unchanged; only pin offsets flip:
        N:  (dx, dy) → ( dx,  dy)
        FN: (dx, dy) → (-dx,  dy)   # mirror horizontal
        FS: (dx, dy) → ( dx, -dy)   # mirror vertical
        S:  (dx, dy) → (-dx, -dy)   # 180°

    Returns
    -------
    orientations : np.ndarray [num_hard_macros] int in {0, 1, 2, 3}
        0=N, 1=FN, 2=FS, 3=S (Klein-4 indexing per the contest spec)
    updated_offsets : list of np.ndarray
        Per-hard-macro pin offsets after flipping (same shape as
    ...
    # Pin offsets per orientation (per macro), pre-computed
    base_offsets = [m.numpy().astype(np.float64) for m in benchmark.macro_pin_offsets]
    sign_table = np.array(
        [[1.0, 1.0],   # N
         [-1.0, 1.0],  # FN
         [1.0, -1.0],  # FS
    ...
    # offsets_per_orient[macro_i][orient] = [num_pins, 2]
    offsets_per_orient = [
        [base * sign_table[o] for o in range(4)] for base in base_offsets
    ]

    # Build per-net pin owner / slot tables
    # For each net, list of (owner, slot) pairs
    net_pins_list = [pins.numpy() for pins in benchmark.net_pin_nodes]
    # For each hard macro, list of (net_idx, pin_idx_in_net) it appears in
    macro_to_net_appearances: List[List[Tuple[int, int]]] = [[] for _ in range(n_hard)]
    for net_idx, pins in enumerate(net_pins_list):
        for pin_idx_in_net, (owner, slot) in enumerate(pins.tolist()):
            if 0 <= owner < n_hard:
                macro_to_net_appearances[owner].append((net_idx, pin_idx_in_net))

    # Compute pin position arrays per net (refreshed when orientations change).
    # net_pin_positions[net_idx] = ndarray [num_pins_in_net, 2]
    def _pin_positions_for_net(net_pins, current_orient, current_offsets):
        pos = np.zeros((len(net_pins), 2), dtype=np.float64)
        for i, (owner, slot) in enumerate(net_pins.tolist()):
            owner = int(owner); slot = int(slot)
            if owner < n_hard:
                pos[i] = placement_full_np[owner] + current_offsets[owner][slot]
            elif owner < n_macros:
                pos[i] = placement_full_np[owner]
            else:
                pos[i] = port_pos[owner - n_macros]
        return pos

    orientations = np.zeros(n_hard, dtype=np.int64)
    current_offsets = [base.copy() for base in base_offsets]

    # Pre-compute per-net pin positions
    net_pin_positions = [
        _pin_positions_for_net(net_pins_list[ni], orientations, current_offsets)
        for ni in range(len(net_pins_list))
    ]

    for pass_idx in range(max_passes):
        n_changed = 0
        for i in range(n_hard):
            appearances = macro_to_net_appearances[i]
            if not appearances:
                continue
            # For each orientation, compute total contribution to HPWL over
            # the nets this macro is in.
            macro_pos = placement_full_np[i]
            best_o = orientations[i]
            best_wl = None
            for o in range(4):
                offs_o = offsets_per_orient[i][o]
                total = 0.0
                for net_idx, pin_idx in appearances:
                    pos = net_pin_positions[net_idx].copy()
                    pos[pin_idx] = macro_pos + offs_o[int(net_pins_list[net_idx][pin_idx, 1])]
                    xmax = pos[:, 0].max(); xmin = pos[:, 0].min()
                    ymax = pos[:, 1].max(); ymin = pos[:, 1].min()
                    total += (xmax - xmin) + (ymax - ymin)
                if best_wl is None or total < best_wl:
                    best_wl = total
                    best_o = o
            if best_o != orientations[i]:
                orientations[i] = best_o
                current_offsets[i] = offsets_per_orient[i][best_o]
                # Update affected nets' pin positions
                for net_idx, pin_idx in appearances:
                    net_pin_positions[net_idx][pin_idx] = (
                        macro_pos + current_offsets[i][int(net_pins_list[net_idx][pin_idx, 1])]
                    )
                n_changed += 1
        if n_changed == 0:
            break

    return orientations, current_offsets


def _apply_orientations_to_plc(plc, benchmark: Benchmark, updated_offsets: List[np.ndarray]):
    """
    Mutate plc's hard-macro pin offsets in place so ``compute_proxy_cost``
    reflects the chosen orientations.

    Each MACRO_PIN node in plc has ``x_offset`` and ``y_offset`` floats; we
    set them to match ``updated_offsets[macro_i][pin_slot]``.
    """
    n_hard = benchmark.num_hard_macros
    for bench_idx in range(n_hard):
        plc_idx = benchmark.hard_macro_indices[bench_idx]
        macro_name = plc.modules_w_pins[plc_idx].get_name()
        # Find all pin children of this macro in plc
        pin_indices = []
        for j, mod in enumerate(plc.modules_w_pins):
            if mod.get_type() == "MACRO_PIN" and hasattr(mod, "get_macro_name"):
                if mod.get_macro_name() == macro_name:
                    pin_indices.append(j)
        # Match plc pin order to our offsets order via macro_pin_offsets[bench_idx]
        # We assume they're in the same order — they were extracted that way in
        # loader.py.  Apply the orientation-flipped offsets.
        flipped = updated_offsets[bench_idx]
        if len(pin_indices) != flipped.shape[0]:
            # Mismatch — skip this macro to be safe
            continue
        for k, pin_idx in enumerate(pin_indices):
            pin = plc.modules_w_pins[pin_idx]
            pin.x_offset = float(flipped[k, 0])
            pin.y_offset = float(flipped[k, 1])
    # Force re-compute on next get_cost call
...
def _build_population_seeds(
    ...
    ch: float,
    seed: int,
    soft_to_hards: Optional[List[List[int]]] = None,
) -> np.ndarray:
    """
    ...
    # Lazily build the soft→connected-hards index if caller didn't pass it.
    if soft_to_hards is None:
        soft_to_hards = _build_soft_to_hards_index(benchmark)

    pop = np.zeros((K, n_macros, 2), dtype=np.float32)

    # Seed 0: legalized initial.plc with ORIGINAL soft positions (the
    # safety-net anchor).  Important to keep this one identical to the proxy's
    # anchor cost reference.
    ...
    # Helper: for a given hard layout, initialise soft macros at their
    # connected-hard centroids (Phase-A improvement; previously canvas centre).
    def _soft_init_for(hard_pos, scale_jit=0.05):
        soft = _init_soft_at_centroids(hard_pos, soft_to_hards, cw, ch)
        soft = soft + rng.standard_normal(soft.shape) * (min(cw, ch) * scale_jit)
        return soft

    n_spec = max(1, K // 8) if K >= 8 else 0
    n_fd = max(1, K // 4) if K >= 4 else 0
    used = 1
    # Seeds 1..n_spec: spectral with random rotations + centroid soft
    for k in range(1, 1 + n_spec):
        ...
        pop[k, :n_hard] = sp_leg
        pop[k, n_hard:] = _soft_init_for(sp_leg, scale_jit=0.05)
        used = k + 1
    # FD-random + centroid soft
    for k in range(used, used + n_fd):
        ...
        pop[k, :n_hard] = fd_leg
        pop[k, n_hard:] = _soft_init_for(fd_leg, scale_jit=0.05)
        used = k + 1
    # Remaining: jittered initial.plc at varying scales
    # Soft macros for these seeds stay near initial.plc soft (since hard is
    # close to initial) — that's already a centroid-like initialisation.
...
class GraphGradPlacer:
    ...
    def __init__(
        ...
        self.orientation_passes = orientation_passes
    ...
    def _place_soft_only(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # ── Phase-A: Klein-4 orientation optimization ──
        # Try each orientation per hard macro, pick the one that minimises
        # that macro's HPWL contribution.  Mutates plc pin offsets so the
        # proxy reflects the new orientations, and writes orientations.pt
        # for Tier-2 (which carries the sidecar to OpenROAD).
        if self.optimize_orientation and best_across is not None and plc_cached is not None:
            from macro_place.objective import compute_proxy_cost
            t_orient = time.time()
            best_np = best_across.numpy().astype(np.float64)
            self._log("orientation polish: starting Klein-4 optimization")
            try:
                orientations, updated_offsets = _optimize_orientations(
                    benchmark, best_np, max_passes=self.orientation_passes
                )
                n_flipped = int((orientations != 0).sum())
                self._log(
                    f"  {n_flipped}/{benchmark.num_hard_macros} macros flipped "
                    f"from default N  ({time.time()-t_orient:.1f}s)"
                )
                # Apply to plc so the proxy benefits, then re-validate true cost.
                _apply_orientations_to_plc(plc_cached, benchmark, updated_offsets)
                c_oriented = compute_proxy_cost(best_across, benchmark, plc_cached)
                self._log(
                    f"  post-orient proxy={c_oriented['proxy_cost']:.4f} "
                    f"(was {best_across_cost:.4f})"
                )
                if c_oriented["overlap_count"] == 0 and c_oriented["proxy_cost"] < best_across_cost:
                    best_across_cost = c_oriented["proxy_cost"]
                # Persist orientations.pt sidecar for Tier-2
                try:
                    out_dir = Path("orientations_out") / benchmark.name
                    out_dir.mkdir(parents=True, exist_ok=True)
                    torch.save(
                        torch.tensor(orientations, dtype=torch.long),
                        out_dir / "orientations.pt",
                    )
                    self._log(f"  saved {out_dir / 'orientations.pt'}")
                except Exception as e:
                    self._log(f"  could not save orientations.pt: {e}")
            except Exception as e:
                self._log(f"  orientation pass failed (continuing): {e}")
